...
{% for s in stories %}
  ...
  <div class="subtle">{{ s.kind }} — {{ s.name }}</div>
...
{% for section, rows in tables.items() %}
  ...
  <th>Δ Loads</th><th>Impact S</th><th>Impact D (000s)</th>
...
def approx_tokens(s: str) -> int:
    return max(1, math.ceil(len(s) / 4))  # rough ≈ 4 chars per token
...
def make_client_highlights_from_network(label_A: str, label_B: str, netA: dict, netB: dict) -> Dict[str, Any]:
    ...
    highlights = []
    highlights.append(f"Load count: {netA['loads']:,} (A) vs {netB['loads']:,} (B) — {arrow(netA['loads']-netB['loads'])}.")
    highlights.append(f"Core OR: {netA['or_pct']:.1f}% (A) vs {netB['or_pct']:.1f}% (B) — {arrow(netA['or_pct']-netB['or_pct'], better_when_down=True, units='pts')}.")
    highlights.append(f"Revenue/Load: {netA['rpl']:,} (A) vs {netB['rpl']:,} (B) — {arrow(netA['rpl']-netB['rpl'])}.")
    highlights.append(f"Overhead/Load: {netA['ohpl']:,} (A) vs {netB['ohpl']:,} (B) — {arrow(netA['ohpl']-netB['ohpl'], better_when_down=True)}.")
    highlights.append(f"Variable/Load: {netA['varpl']:,} (A) vs {netB['varpl']:,} (B) — {arrow(netA['varpl']-netB['varpl'], better_when_down=True)}.")
...
def make_final_overview(netA: dict, netB: dict, tables: dict) -> str:
    ...
    top = movers[:3]
    pieces = []
    pieces.append(f"Overall, {direction} from Period B to A (Core OR {netB['or_pct']:.1f}% → {netA['or_pct']:.1f}%).")
    if top:
...
# ---------------------------------------------------
# NARRATIVE (LLM) — your full system_instructions
# ---------------------------------------------------
...
- If N ≥ 8: Period A = last 4 weeks; Period B = prior 4 weeks.
- If 5–7:   Period A = last 1 week;  Period B = prior min(4, N−1) weeks (averaged).
- If 3–4:   Period A = last 1 week;  Period B = all prior weeks (averaged).
- If 2:     Period A = last 1 week;  Period B = prior 1 week.

Weeks are Sunday–Saturday; each week is identified by its Saturday date.
...
- MPL = (Revenue − Total Cost) / Loads; RPL = Revenue / Loads.
- Impact_S = (OR_B − OR_A)×Avg_Loads + (Network_OR_B − OR_B)×ΔLoads
- Impact_D = (MPL_A − MPL_B)×Avg_Loads + (MPL_B − Network_MPL_B)×ΔLoads
- Dollarized_S = Impact_S × Avg_RPL; report **Impact_D** and **Dollarized_S** in **(000s)** with 2 decimals.
- Composite (000s) = Impact_D(000s) + w_S × Dollarized_S(000s), with:
  f_quality = min(|OR_B − Network_OR_B|×100/5, 1.0),
  w_stability = sqrt(Avg_Loads / (Avg_Loads + L0)),
  w_S = 0.5 × f_quality × w_stability.
- Conflict guardrail: if Impact_D and Dollarized_S disagree, |$S| < 2×|D|, and their sum would flip D’s sign, snap Composite’s sign to match Impact_D (keep sum’s magnitude).
- Filter: Exclude a group ONLY if BOTH periods have loads ≤ 0.75% of that period’s network loads.
- Substitution: If a group has 0 loads in a period, the tool substitutes Period-B network OR/MPL/RPL for that period; Core OR cells show “*” in the internal table. Only mention “compared against network Core OR” in a story when substitution actually occurred for that entity/period.

## Executive summary formatting
- Columns use Saturday labels with window size: e.g., “08/30/25 (1wk)” vs “08/23/25 (4wk)”.
- “Change” column uses arrows: ▲ positive (helped), ▼ negative (hurt), → neutral.
...
- Remove “Avg Loads” column. Show Core OR values with “*” where substitution happened.
...
- Begin each story with an arrow: ▲ helped, ▼ hurt, → neutral.
- Tone is conversational: say “Core OR improved” when OR% decreased; “worsened” when OR% increased; “held” when flat.
- Mention “compared against network Core OR” only when a substitution occurred for that entity/period.
- Include a short driver detail line:
  • If story entity is a **Customer** → identify the **Lane** with the largest |Composite|.
  • If **Lane** → identify the most impactful **Bill To (customer)**.
  • If **Area** (Outbound/Inbound) → identify whichever (**Lane** or **Customer**) has the largest |Composite| within that area (or closest proxy provided by the tool).

## Client-facing FINAL WORD (new)
After the Top-10 stories, add a **1–4 sentence “final overview”** that gives the clear headline on profitability from Period B → Period A. Address direction (helped/hurt/flat), briefly cite the largest driver types (e.g., OR improvement vs margin/volume shifts), and keep it concise and plain-English. Do not restate every number; synthesize what changed and why it mattered.
...
def build_client_and_internal_reports(client: OpenAI, model: str, calc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    client_report = {
        ...
        "style_key": (data.get("client_report", {}) or {}).get("style_key", [
            "▲ helped network profitability", "▼ hurt network profitability", "→ neutral",
            "Say “Core OR improved” when OR% decreased; “worsened” when OR% increased; “held” when flat.",
            "Mention “compared against network Core OR” only when asterisks indicate a substituted network value."
        ])
    }
...
def main():
    ...
    print("Auth preflight:", "key="+(key[:12]+"…"), "project="+(proj or "<none>"), "org="+(org or "<none>"))
    ...
    if args.pdf:
        ...
        print("🖨 Rendered:")
        print("  ", client_html_out, "→", (client_pdf_out if client_pdf_ok else "open HTML and Print to PDF"))
        print("  ", internal_html_out, "→", (internal_pdf_out if internal_pdf_ok else "open HTML and Print to PDF"))

    # token hint
    est_in_tokens = approx_tokens(json.dumps({"network": calc["network"], "top10": calc["stories_top10"]}))
    print(f"ℹ️ Rough input tokens to model: ~{est_in_tokens:,}")
